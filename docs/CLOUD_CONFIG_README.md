...
### 1. 三级优先级查询链
- **最高优先级**: UserFeatureMappings (用户自定义单语言覆盖)
- **中等优先级**: CloudFeatureMappings (云端多语言配置)
- **最低优先级**: App/OplusFeatureMappings (应用内置资源)
...
### 描述查询流程
1. 首先查询用户自定义映射 (UserFeatureMappings)
2. 如果不存在，查询云端配置映射 (CloudFeatureMappings)
3. 如果仍不存在，使用应用内置资源映射
4. 最终回退到特性名称本身
